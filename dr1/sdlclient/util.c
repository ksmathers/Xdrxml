...
int isfloor( dr1Map *map, int row, int col) {
    dr1MapGraphic *gr = map->grid[row * map->xsize + col].graphic;
    int g;
    if (!gr) return FALSE;
...
int dr1_los(int y1, int x1, int y2, int x2, dr1Map *map)
{
    /* Delta */
    int dx, dy;
...
#       define ABS(x) ((x)<0?-(x):(x))
    ...
    if (!dx)
    {
        ...
        if (dy > 0)
        {
            ...
            {
                if (!isfloor(map, ty, x1)) return FALSE;
            }
        }
    ...
    if (!dy)
    {
        ...
        if (dx > 0)
        {
            ...
            {
                if (!isfloor(map, y1, tx)) return (FALSE);
            }
        }
    ...
    if (ax == 1)
    {
        if (ay == 2)
        {
            if (isfloor(map, y1 + sy, x1)) return (TRUE);
        }
    }
    ...
    else if (ay == 1)
    {
        if (ax == 2)
        {
            if (isfloor(map, y1, x1 + sx)) return (TRUE);
        }
    }
    ...
    if (ax >= ay)
    {
        ...
        while (x2 - tx)
        {
            if (!isfloor(map, ty, tx)) return (FALSE);
